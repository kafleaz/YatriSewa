...
namespace YatriSewa.Controllers
{
    public class LoginController : Controller
    {
        ...
        //        ModelState.AddModelError(string.Empty, "Please enter a valid 6-digit OTP.");
        //        TempData.Keep("EmailOrPhone"); // Keep TempData value for email/phone display
        //        TempData.Keep("NewPassword");
        //        return View("Verification");  // Return the same page with the error
        //    }

        //    // Retrieve email or phone from session or TempData
        //    var emailOrPhone = HttpContext.Session.GetString("PhoneEmail") ?? TempData.Peek("EmailOrPhone")?.ToString();
        //    if (string.IsNullOrEmpty(emailOrPhone))
        //    {
        //        ModelState.AddModelError(string.Empty, "Session expired, please try again.");
        //        return RedirectToAction("SignUp");
        ...
        //    {
        //        ModelState.AddModelError(string.Empty, "Incorrect OTP. Please try again.");
        //        TempData.Keep("EmailOrPhone");  // Keep TempData alive for the next request
        //        TempData.Keep("NewPassword");   // Keep TempData alive for the next request
        //        return View("Verification");
        //    }

        //    // Check if this is a password reset request or a registration verification
        //    bool isPasswordReset = (bool?)TempData["IsPasswordReset"] ?? false;

        //    if (isPasswordReset)
        //    {
        //        // If it's for password reset, update the password
        ...
        //    else
        //    {
        //        // If it's for registration, verify the user
        //        user.IsVerified = true;
        //        user.OTP = 0;  // Clear OTP after successful verification
        //        _context.User_Table.Update(user);
        //        await _context.SaveChangesAsync();

        //        TempData["SuccessMessage"] = "Your account has been verified successfully. Please log in.";
        //        return RedirectToAction("SignIn");
        //    }
        //}
        ...
        //    {
        //        ModelState.AddModelError(string.Empty, "Please enter a valid 6-digit OTP.");
        //        TempData.Keep("EmailOrPhone"); // Ensure TempData persists
        //        TempData.Keep("NewPassword");  // Ensure TempData persists
        //        return View("Verification");
        //    }

        //    // Retrieve email or phone from session or TempData
        //    var emailOrPhone = HttpContext.Session.GetString("PhoneEmail") ?? TempData.Peek("EmailOrPhone")?.ToString();  // Use Peek to retain TempData
        //    if (string.IsNullOrEmpty(emailOrPhone))
        //    {
        //        ModelState.AddModelError(string.Empty, "Session expired, please try again.");
        //        return RedirectToAction("SignUp"); // Redirecting incorrectly happens here if emailOrPhone is lost
        //    }
        ...
        //    // Check if this is a password reset request or a registration verification
        //    bool isPasswordReset = (bool?)TempData.Peek("IsPasswordReset") ?? false;

        //    if (isPasswordReset)
        //    {
        //        // If it's for password reset, retrieve the new password from TempData
        //        var newPassword = TempData.Peek("NewPassword")?.ToString();
        //        if (string.IsNullOrEmpty(newPassword))
        //        {
        //            ModelState.AddModelError(string.Empty, "Session expired. Please try again.");
        //            return RedirectToAction("ForgotPassword");
